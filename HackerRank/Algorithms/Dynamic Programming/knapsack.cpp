...
int get_max_profit_bottom_up(const std::vector<unsigned> &profit,
                             const std::vector<unsigned> &weight, int capacity)
{
    std::vector<int> solution(capacity + 1, 0);

    for (size_t avail_capacity = 0; avail_capacity <= capacity;
         ++avail_capacity) {
        if (weight.front() <= avail_capacity) {
            solution[avail_capacity] = static_cast<int>(profit.front());
        }
    }

    for (size_t i = 1; i < profit.size(); ++i) {
        for (size_t avail_capacity = capacity; avail_capacity <= capacity;
             --avail_capacity) {
            int profit1{0};
            int profit2{0};
            ...
            if (weight[i] <= avail_capacity) {
                profit1 = static_cast<int>(profit[i]) +
                          solution[avail_capacity - weight[i]];
            }

            profit2 = solution[avail_capacity];
            solution[avail_capacity] = std::max(profit1, profit2);
        }
    }
